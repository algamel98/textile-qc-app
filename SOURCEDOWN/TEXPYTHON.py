...
Original file is located at
    https://colab.research.google.com/drive/1Pk01wBSQiVWH2yAX7OarGR71e08i87kT
"""
...
DPI = 300
DEFAULT_TIMEZONE_OFFSET_HOURS = 3  # Default timezone offset (UTC+3 for Turkey)
...
def get_local_time(timezone_offset=None):
    """Get current time with configurable timezone offset.

    Args:
        timezone_offset: Hours offset from UTC. If None, uses DEFAULT_TIMEZONE_OFFSET_HOURS.

    Returns:
        datetime: Current time adjusted for timezone
    """
...
def make_header_footer(report_timestamp=None, analysis_id=None):
    ...
    def header_footer(canvas_, doc):
        ...
        if pno >= 2:
            ...
            # Show page number and analysis ID
            page_text = f"Page {pno}"
            if analysis_id:
                page_text = f"{analysis_id} | {page_text}"
            canvas_.drawRightString(width - MARGIN_R, fy-2, page_text)
...
def run_pipeline_and_build_pdf(ref_path, test_path, ref, test, settings):
    ...
    exec_summary_header = Paragraph(f"<b>📊 EXECUTIVE SUMMARY: {decision}</b>", exec_summary_style)
    ...
    exec_data = [
        ["Metric", "Score", "Status"],
        ["Color Score", f"{color_score:.1f}/100", color_status_str],
        ["Pattern Score (SSIM)", f"{pattern_score:.1f}/100", pattern_status_str],
        ["ΔE2000 (Mean)", fmt2(mean_de00_D65), status_color],
        ["Overall Score", f"{overall_score:.1f}/100", decision.split()[0]]
    ]

    # Add pattern repetition if enabled
    if settings.enable_pattern_repetition and cc_ref is not None:
        exec_data.insert(3, ["Pattern Repetition", f"{integrity_assessment['integrity_score']:.1f}%", pattern_rep_status])
    ...
    if settings.enable_analysis_settings:
        settings_section = []
        settings_section.append(Paragraph("<b>Analysis Settings</b>", StyleH1))
        settings_section.append(Spacer(1, 6))
        settings_section.append(Paragraph("The following settings were used for this analysis:", StyleBody))
        settings_section.append(Spacer(1, 6))
    ...
    if settings.enable_color_unit:
        ...
        if settings.enable_color_input_images:
            input_section = []
            input_section.append(Paragraph("Input Images", StyleH2))
            img_row = [
        ...
        if settings.enable_color_measurements:
            ...
            rgb_section = []
            rgb_section.append(Paragraph("Color Measurements", StyleH2))
            rgb_section.append(Paragraph("5-point regional analysis with Reference vs Sample comparison", StyleSmall))
            ...
            rgb_section.append(Paragraph("<b>RGB Color Values</b>", StyleBody))
            ...
            lab_section = []
            lab_section.append(Paragraph("<b>LAB* Color Space Values</b>", StyleBody))
            ...
            xyz_section = []
            xyz_section.append(Paragraph("<b>XYZ Tristimulus Values</b>", StyleBody))
            xyz_cols = ["Region", "Ref X", "Ref Y", "Ref Z", "Test X", "Test Y", "Test Z"]
            xyz_tbl = [xyz_cols]
            for _,r in df_samples.iterrows():
                row = [int(r["Region"]),
            ...
            # Table 4: CMYK Values
            cmyk_section = []
            cmyk_section.append(Paragraph("<b>CMYK Color Values</b>", StyleBody))
            cmyk_cols = ["Region", "Ref C%", "Ref M%", "Ref Y%", "Ref K%", "Test C%", "Test M%", "Test Y%", "Test K%"]
            cmyk_tbl = [cmyk_cols]
            for _,r in df_samples.iterrows():
                row = [int(r["Region"]),
            ...
            de_section = []
            de_section.append(Paragraph("<b>Color Difference Metrics</b>", StyleBody))
            ...
            de_interpretation = ("Not perceptible" if mean_de00_D65 < 1.0 else
            ...
            de_summary_section = []
            de_summary_section.append(Paragraph("<b>ΔE Summary Statistics</b>", StyleBody))
            de_summary = [["Metric", "Mean", "Std Dev", "Min", "Max", "Overall Status"],
                         ...
                         ["ΔE2000", fmt2(mean_de00_D65), fmt2(float(np.std(de00_map))),
                          fmt2(float(np.min(de00_map))), fmt2(float(np.max(de00_map))), de_overall_status]]
            ...
            de_summary_section.append(Paragraph(f"<i>Interpretation: {de_interpretation}</i>", StyleSmall))
        ...
        if settings.enable_color_statistical:
            stats_section = []
            stats_section.append(Paragraph("Statistical Analysis (RGB)", StyleH2))
            ...
            quality_section = []
            quality_section.append(Paragraph("Color Quality Indices", StyleH2))
            ...
            quality_indices = [["Index", "Value", "Status", "Interpretation"],
                              ["Metamerism Index", fmt2(metamerism_index), meta_status,
                               "Color consistency across D65/TL84/A illuminants"],
                              ["Uniformity Index", fmt1(uni_idx)+"/100", uni_status,
                               "Spatial consistency of color across the sample"]]
        ...
        if settings.enable_color_spectral_proxy:
            ...
            spectral_proxy_section.append(Paragraph("Spectral Analysis (Proxy)", StyleH2))
            spectral_proxy_section.append(Paragraph("The chart approximates spectral behavior from RGB averages to aid visual comparison.", StyleSmall))
        ...
        if settings.enable_color_lab_detailed:
            lab_section = []
            lab_section.append(Paragraph("Detailed Lab* Color Space Analysis", StyleH2))
        ...
        if settings.enable_color_lab_viz:
            ...
            lab_viz_elements.append(Paragraph("a*-b* chromaticity plot and L*a*b* component comparison.", StyleSmall))
        ...
        if settings.enable_color_quality_assessment:
            qa_section = []
            qa_section.append(Paragraph("Quality Assessment (Lab* thresholds)", StyleH2))
        ...
        if settings.enable_color_recommendations:
            rec_section = []
            rec_section.append(Paragraph("Recommendations (Based on Lab*)", StyleH2))
            recs = []
            ...
            rec_tbl = [["Parameter", "Action"]] + [[param, wrap_text_cell(action, 60)] for param, action in recs]
    ...
    if settings.enable_pattern_unit:
        ...
        if settings.enable_pattern_ssim:
            ...
            pattern_metrics_section.append(Paragraph("Pattern Metrics", StyleH2))
        ...
        hist_section = []
        hist_section.append(Paragraph("Histograms (RGB)", StyleH2))
        ...
        hist_interp = """<i>Interpretation: RGB histograms show the distribution of color values across the image.
        Similar histogram shapes between Reference and Sample indicate consistent color reproduction.
        Shifts in peak positions suggest color bias; narrower distributions indicate more uniform color.</i>"""
        ...
        if settings.enable_pattern_advanced:
            ...
            fft_section = []
            fft_section.append(Paragraph("Fourier Domain Analysis", StyleH2))
            fft_section.append(Paragraph("2D Fast Fourier Transform reveals periodic structures and directional patterns in the fabric.", StyleSmall))
            ...
            # B. Gabor Filter Bank
            gabor_section = []
            gabor_section.append(Paragraph("Gabor Filter Bank Analysis", StyleH2))
            gabor_section.append(Paragraph("Multi-scale and multi-orientation responses capture texture at different frequencies and angles.", StyleSmall))
            ...
            glcm_section = []
            glcm_section.append(Paragraph("GLCM Texture Features", StyleH2))
            glcm_section.append(Paragraph("Gray Level Co-occurrence Matrix (GLCM) quantifies spatial relationships in texture.", StyleSmall))
            ...
            lbp_section = []
            lbp_section.append(Paragraph("Local Binary Patterns (LBP)", StyleH2))
            lbp_section.append(Paragraph("LBP captures local texture by encoding pixel neighborhoods into binary patterns.", StyleSmall))
            ...
            wavelet_section = []
            wavelet_section.append(Paragraph("Wavelet Decomposition", StyleH2))
            wavelet_section.append(Paragraph(f"Multiresolution analysis using {settings.wavelet_type} wavelet at {settings.wavelet_levels} levels.", StyleSmall))
            ...
            struct_section = []
            struct_section.append(Paragraph("Structure Tensor Analysis", StyleH2))
            ...
            if len(struct_test['orientation_degrees']) > 0:
                struct_section.append(Paragraph("Line Angle Distribution", StyleSmall))
            ...
            defect_section = []
            defect_section.append(Paragraph("Defect Detection & Saliency Map", StyleH2))
            defect_section.append(Paragraph("Spectral residual saliency combined with morphological operations identifies potential defects.", StyleSmall))
            ...
            if defects_analysis['defects']:
                ...
                defect_section.append(Paragraph(f"Total defects detected: {defects_analysis['defect_count']}", StyleSmall))
            else:
                defect_section.append(Paragraph("No significant defects detected.", StyleBody))
    ...
    if settings.enable_pattern_repetition and cc_ref is not None:
        ...
        elements.append(Paragraph("Analysis of pattern count, distribution, and integrity.", StyleSmall))
        elements.append(Spacer(1, 8))

        # A. Pattern Detection Summary
        if settings.enable_pattern_rep_summary:
            summary_section = []
            summary_section.append(Paragraph("Pattern Detection Summary", StyleH2))
        ...
        if settings.enable_pattern_rep_count:
            count_section = []
            count_section.append(Paragraph("Pattern Count Analysis", StyleH2))
            count_section.append(Paragraph("Detected patterns in reference and sample images using connected components analysis.",
                                    StyleSmall))
        ...
        if settings.enable_pattern_rep_blob:
            blob_section = []
            blob_section.append(Paragraph("Blob Detection Results", StyleH2))
            blob_section.append(Paragraph("SimpleBlobDetector analysis with circularity and convexity filtering.", StyleSmall))
        ...
        if settings.enable_pattern_rep_keypoint and keypoint_matching:
            kp_section = []
            kp_section.append(Paragraph("Keypoint Matching Analysis", StyleH2))
            kp_section.append(Paragraph(f"Feature-based matching using {settings.keypoint_detector} detector.", StyleSmall))
        ...
        if settings.enable_pattern_rep_autocorr and autocorr_test:
            autocorr_section = []
            autocorr_section.append(Paragraph("Auto-correlation Analysis", StyleH2))
            autocorr_section.append(Paragraph("2D auto-correlation reveals pattern periodicity and regularity.", StyleSmall))
        ...
        if settings.enable_pattern_rep_spatial and spatial_test:
            spatial_section = []
            spatial_section.append(Paragraph("Spatial Distribution Analysis", StyleH2))
            spatial_section.append(Paragraph(f"Grid-based pattern density analysis (cell size: {settings.grid_cell_size}px).",
                                    StyleSmall))
        ...
        if settings.enable_pattern_rep_integrity and integrity_assessment:
            integrity_section = []
            integrity_section.append(Paragraph("Pattern Integrity Assessment", StyleH2))
            integrity_section.append(Paragraph("Multi-dimensional comparison of pattern properties.", StyleSmall))
        ...
        if settings.enable_pattern_rep_catalog and missing_extra:
            catalog_section = []
            catalog_section.append(Paragraph("Missing/Extra Patterns Catalog", StyleH2))
            ...
            if missing_extra['missing_patterns']:
                missing_section = []
                missing_section.append(Paragraph(f"<b>Missing Patterns ({missing_extra['missing_count']})</b>", StyleBody))
                ...
                if missing_extra['missing_count'] > 15:
                    missing_section.append(Paragraph(f"... and {missing_extra['missing_count'] - 15} more missing patterns.", StyleSmall))
            ...
            if missing_extra['extra_patterns']:
                extra_section = []
                extra_section.append(Paragraph(f"<b>Extra Patterns ({missing_extra['extra_count']})</b>", StyleBody))
                ...
                if missing_extra['extra_count'] > 15:
                    extra_section.append(Paragraph(f"... and {missing_extra['extra_count'] - 15} more extra patterns.", StyleSmall))
            ...
            # Recommendations
            rep_rec_section = []
            rep_rec_section.append(Paragraph("Pattern Repetition Recommendations", StyleH2))
            recs = []
    ...
    if settings.enable_spectrophotometer:
        ...
        if settings.enable_spectro_config:
            config_section = []
            config_section.append(Paragraph("Instrument Configuration", StyleH2))
        ...
        if settings.enable_spectro_color_diff_methods:
            color_diff_section = []
            color_diff_section.append(Paragraph("Color Difference Methods", StyleH2))
        ...
        if settings.enable_spectro_whiteness:
            wy_section = []
            wy_section.append(Paragraph("Whiteness & Yellowness Indices", StyleH2))
        ...
        if settings.enable_spectro_metamerism:
            meta_section = []
            meta_section.append(Paragraph("Metamerism Analysis", StyleH2))
            meta_section.append(Paragraph("Color difference under various illuminants to assess metamerism.", StyleSmall))
            if metamerism_results:
                ...
                if worst_metamerism:
                    meta_section.append(Spacer(1, 4))
                    meta_section.append(Paragraph(f"<b>Worst-case metamerism:</b> {worst_metamerism['illuminant']} (ΔE = {fmt2(worst_metamerism['delta_e'])})", StyleBody))
        ...
        if settings.enable_spectro_calibration:
            calib_section = []
            calib_section.append(Paragraph("Calibration & Limitations", StyleH2))
