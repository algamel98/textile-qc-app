# -*- coding: utf-8 -*-
"""
QCSettings dataclass for Textile QC System
"""
from dataclasses import dataclass, field
import numpy as np
from app.core.constants import DEFAULT_TIMEZONE_OFFSET_HOURS
...
>>>>>>> c6f070ccf06e1e0ba40d88c583b5a4834e7ad443
    ...
    # Region selection
    use_crop: bool = False
    crop_shape: str = "circle"
    crop_mode: str = "simultaneous"
    crop_center_x: int = 0
    crop_center_y: int = 0
    crop_diameter: int = 500
    crop_width: int = 500
    crop_height: int = 500
    crop_test_center_x: int = 0
    crop_test_center_y: int = 0
    crop_test_diameter: int = 500
    crop_test_width: int = 500
    crop_test_height: int = 500
    ...
    geometry_mode: str = "d/8 SCI"
    use_delta_e_cmc: bool = True
    cmc_l_c_ratio: str = "2:1"
    ...
    def from_dict(cls, data):
        ...
        for key, value in data.items():
            if hasattr(settings, key):
                if key.startswith('spectral_') and key.endswith(('wavelengths', 'reflectance')):
                    setattr(settings, key, np.array(value) if value else np.array([]))
                elif key in ('gabor_frequencies', 'glcm_distances', 'glcm_angles', 'metamerism_illuminants'):
                    setattr(settings, key, value if isinstance(value, list) else [])
                else:
